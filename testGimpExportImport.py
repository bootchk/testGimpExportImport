...
def test_file_format(image, drawable, format_moniker):
    ...
    (when there is a Gimp.PDB.file_foo_save procedure)
    and file not exist.
    TODO when file already exists, do we test the saver?

    Then try loading, if file exists and a loader exists.
...
register(
      ...
      [(PF_BOOL, "result", "Whether test(s) passed.", True)],
      plugin_func,
      menu="<Image>/Test")
main()
